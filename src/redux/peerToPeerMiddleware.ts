...
const peerToPeerMiddleware = <S>({
  ...
  return (api: MiddlewareAPI<S>) => (next: Dispatch<S>) => (
    ...
    if (commsNode && !action.fromPeerId && typeof action === "object") {
      ...
      if (sendToOptions) {
        commsNode.sendTo(
          { ...action, fromPeerId: commsNode.peerId },
          sendToOptions
        )
      }
    }
