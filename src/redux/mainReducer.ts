...
import loggedInUserReducer, {
  LoggedInUserReducerType
} from "./loggedInUserReducer"
...
export default function buildStore() {
  ...
  } = connectRoutes<{}, LocationState>(
    createHistory({ basename: "/gtove" }),
    routesMap
  )
    ...
    switch (action.type) {
      case DISCARD_STORE:
        return combinedReducers(
          { location: state.location } as ReduxStoreType,
          action
        )
      default:
  ...
  const onSentMessage = (recipients: string[], action: object) => {
    ...
    if (scenario) {
      store.dispatch(
        setLastCommonScenarioAction(
          scenario,
          action as ScenarioReducerActionType
        )
      )
    }
  }
  ...
  const gTovePeerToPeerMiddleware = peerToPeerMiddleware<ReduxStoreType>({
    getCommsChannel: state => ({
      commsChannelId:
        getLoggedInUserFromStore(state) &&
        getTabletopFromStore(state).gm &&
        getTabletopIdFromStore(state),
    ...
    peerNodeOptions: {
      onEvents: [
        {
          event: "connect",
          callback: (peerNode, peerId) => {
            const loggedInUser = getLoggedInUserFromStore(store.getState())
            if (loggedInUser) {
              peerNode.sendTo(
                addConnectedUserAction(peerNode.peerId, loggedInUser),
                { only: [peerId] }
              )
            }
          }
        ...
        {
          event: "data",
          callback: (peerNode, peerId, data) => {
            const action = JSON.parse(data)
            store.dispatch(action)
            store.dispatch(
              setLastCommonScenarioAction(
                getScenarioFromStore(store.getState()),
                action as ScenarioReducerActionType
              )
            )
            // Handle challenge/response actions for other users claiming to be GMs.
            const challengeAction = handleChallengeActions(
              action,
              store.getState()
            )
            if (challengeAction) {
    ...
    getSendToOptions: (action: AnyAction) => {
      if (action.peerKey) {
        ...
        if (action.gmOnly) {
          ...
          const gmClientPeerIds = Object.keys(connectedUsers).filter(
            peerId => connectedUsers[peerId].verifiedGM
          )
  ...
  const gToveFirebaseMiddleware = firebaseMiddleware({
    syncToClient: (setScenarioAction: any, firebaseRef: Promise<any>) => {
      return firebaseRef.then((snapshot: any) => {
        ...
        if (loadedScenario) store.dispatch(setScenarioAction(loadedScenario))
      })
    }
  })
...
export function getLoggedInUserFromStore(
  store: ReduxStoreType
): LoggedInUserReducerType {
  return store.loggedInUser
}

export function getAllFilesFromStore(
  store: ReduxStoreType
): FileIndexReducerType {
  return store.fileIndex
}

export function getConnectedUsersFromStore(
  store: ReduxStoreType
): ConnectedUserReducerType {
  return store.connectedUsers
}

export function getMyPeerIdFromStore(
  store: ReduxStoreType
): MyPeerIdReducerType {
...
export function getTabletopValidationFromStore(
  store: ReduxStoreType
): TabletopValidationType {
