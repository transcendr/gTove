...
export interface MiniType<T = MiniAppProperties | TemplateAppProperties>
  extends WithMetadataType<T> {
  name: string
  position: ObjectVector3
...
function updateMetadata(
  ...
  object: { [key: string]: WithMetadataType<TabletopObjectAppProperties> },
  converter: (
    appProperties: TabletopObjectAppProperties
  ) => TabletopObjectAppProperties
) {
  Object.keys(object).forEach(id => {
    const metadata = fullDriveMetadata[object[id].metadata.id] as DriveMetadata<
      TabletopObjectAppProperties
    >
    if (metadata) {
      object[id] = {
        ...
        metadata: {
          ...metadata,
          appProperties: converter(metadata.appProperties)
        }
      }
    }
  })
...
export function getAllScenarioMetadataIds(scenario: ScenarioType): string[] {
  const metadataMap = Object.keys(scenario.maps).reduce(
    (all, mapId) => {
